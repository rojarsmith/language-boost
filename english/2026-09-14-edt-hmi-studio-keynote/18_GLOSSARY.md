...
| English | 中文 | 最簡單的英文解釋 |
|---|---|---|
| AI-assisted | AI 輔助 | A person stays in control while AI helps with the work. |
| prompt | 提示詞／指令 | The instruction given to the AI assistant. |
| provider | 模型服務提供者 | The service that runs the AI model. |
| local inference | 本機推論 | Running the model on a local machine or approved local host. |
| external provider | 外部服務提供者 | A service outside the local environment that receives the request. |
| edit in place | 原地修改 | Changing existing project content instead of replacing the project. |
| one-step undo | 一步復原 | Reversing the applied AI change with one undo action. |
| human review | 人工審查 | A person checks the AI result before relying on it. |
...
| English | 中文 | 最簡單的英文解釋 |
|---|---|---|
| North Star | 長期方向目標 | A strategic direction, not automatically a forecast. |
| scenario model | 情境模型 | A calculation built from assumptions. |
| approved guidance | 公司核准財測／指引 | A financial statement formally approved for external use. |
| annual run-rate | 年化營收速度 | The yearly level implied by the current operating scale. |
| recurring revenue | 經常性營收 | Revenue supported by contracts or repeated billing. |
| repeatable annual revenue | 可重複的年度營收 | Revenue expected to repeat through orders or renewals, without claiming a guaranteed contract. |
| ARR | 年度經常性收入 | Annual recurring revenue, normally based on recurring contracts. |
| design-in | 設計導入 | A customer's decision to use a component in a product. |
| conversion rate | 轉換率 | The share moving from one funnel stage to the next. |
| renewal | 續約 | A customer continues a service or agreement. |
| gross margin | 毛利率 | Revenue minus direct cost, expressed as a percentage of revenue. |
