...
> The public Alpha exists to validate the product and partner fit with key stakeholders. The core workflow can be demonstrated today, and the limitations are explicit. We will not pass the current Alpha to downstream customers or present roadmap work as complete.

### 4. The source version is newer than the public Alpha. Which version is real?

> Both describe different stages. The development branch is newer and helps us verify engineering progress. The official page defines the public evaluation package, audience, and download terms. For availability, I will always use the official page and approved release statement.
